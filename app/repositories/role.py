...
class PermissionRepository(BaseRepository[Permission]):
    ...
    async def get_by_code(self, code: str) -> Permission | None:
        ...
        result = await self.db.execute(
            select(Permission).where(Permission.code == code, Permission.deleted == 0)
        )
    ...
    async def get_by_ids(self, ids: list[int]) -> list[Permission]:
        ...
        if not ids:
            return []
        result = await self.db.execute(
            select(Permission).where(Permission.id.in_(ids), Permission.deleted == 0)
        )
    ...
    async def get_by_module(self, module: str) -> list[Permission]:
        """根据模块获取权限列表"""
        result = await self.db.execute(
            select(Permission).where(Permission.module == module, Permission.deleted == 0)
        )
        return list(result.scalars().all())
...
class RoleRepository(BaseRepository[Role]):
    ...
    async def get_by_code(self, code: str) -> Role | None:
        ...
        result = await self.db.execute(
            ...
            .where(Role.code == code, Role.deleted == 0)
        )
        return result.scalar_one_or_none()
    ...
    async def get_by_id_with_permissions(self, id: int) -> Role | None:
        ...
        result = await self.db.execute(
            select(Role)
            .options(selectinload(Role.permissions))
            .where(Role.id == id, Role.deleted == 0)
        )
        return result.scalar_one_or_none()
    ...
    async def get_by_ids(self, ids: list[int]) -> list[Role]:
        ...
        result = await self.db.execute(
            select(Role)
            .options(selectinload(Role.permissions))
            .where(Role.id.in_(ids), Role.deleted == 0)
        )
        return list(result.scalars().all())
