...
async def get_role(role_id: int, db: DBSession):
    """获取单个角色详情 - 需要 role:read 权限"""
    role_service = RoleService(db)
...
async def create_role(role_data: RoleCreate, db: DBSession):
...
async def update_role(role_id: int, role_data: RoleUpdate, db: DBSession):
...
async def delete_role(role_id: int, db: DBSession):
...
@permission_router.get(
    ...
    dependencies=[Depends(require_permission(PermissionCode.PERMISSION_READ))],
)
async def get_permissions(db: DBSession):
    """获取所有权限 - 需要 permission:read 权限"""
...
@permission_router.get(
    "/{permission_id}",
    response_model=BaseResponse[PermissionResponse],
    dependencies=[Depends(require_permission(PermissionCode.PERMISSION_READ))],
)
async def get_permission(permission_id: int, db: DBSession):
    """获取单个权限详情 - 需要 permission:read 权限"""
    permission_service = PermissionService(db)
    permission = await permission_service.get_permission(permission_id)
    return BaseResponse(success=True, code=200, msg="获取权限成功", data=PermissionResponse.model_validate(permission))
...
async def create_permission(permission_data: PermissionCreate, db: DBSession):
    """创建新权限 - 需要 permission:create 权限"""
    permission_service = PermissionService(db)
    permission = await permission_service.create_permission(permission_data)
    return BaseResponse(success=True, code=201, msg="创建权限成功", data=PermissionResponse.model_validate(permission))
...
async def update_permission(permission_id: int, permission_data: PermissionUpdate, db: DBSession):
    """更新权限 - 需要 permission:update 权限"""
    permission_service = PermissionService(db)
    permission = await permission_service.update_permission(permission_id, permission_data)
    return BaseResponse(success=True, code=200, msg="更新权限成功", data=PermissionResponse.model_validate(permission))
...
async def delete_permission(permission_id: int, db: DBSession):
